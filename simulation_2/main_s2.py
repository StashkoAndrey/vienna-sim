...
from parameters import (
    GRID_SIZE, MAX_STEPS, DIFFUSION_RATE,
    INITIAL_NUTRIENT, REPLICATES,
    MEAL_INTERVAL, MEAL_TOTAL, CHANNEL_COST
)
...
def run_simulation(channel_cost):
    ...
    for step in range(MAX_STEPS):
        ...
        # ─── Meal at the very start of the step ────────────────────────
        if step % MEAL_INTERVAL == 0 and step != 0:
            grid.refill_nutrients(MEAL_TOTAL)
            logging.info(
                f"Step {step} (meal!), "
                f"Total Nutrients {grid.total_nutrients():.2f}"
            )

        # ─── Now run uptake/divide/death with the fresh nutrients ───────
        grid.step(step)
        grid.nutrients = diffuse(grid.nutrients, DIFFUSION_RATE)

        # ─── Then log state after that step ─────────────────────────────
        count = grid.count_bacteria()
        logging.info(
            f"Step {step}, "
            f"Bacteria {count}, "
            f"Total Energy {grid.total_energy():.2f}, "
            f"Total Nutrients {grid.total_nutrients():.2f}"
        )
...
def main():
    # Run multiple replicates if desired
    all_populations = []
    all_channel_time_series = []
    for rep in range(REPLICATES):
        pop, series = run_simulation(CHANNEL_COST)
    ...
    fig, axes = plt.subplots(len(bins), 1, figsize=(10, 3*len(bins)), sharex=True)
    if len(bins)==1:
        axes=[axes]
