...
# === Настройки ===
CLUSTER_METHOD = 'HDBSCAN_cluster'  # можно заменить на 'GMM_cluster'
...
# === Нормализация вручную с учётом масштаба параметров ===
param_max = {
    'REFILL_INTERVAL_A':1000,'REFILL_INTERVAL_B':1000,'REFILL_INTERVAL_C':1000,'DELTA_T':1000,
    'MEAL_TOTAL_A':1000,'MEAL_TOTAL_B':1000,'MEAL_TOTAL_C':1000,'INITIAL_NUTRIENT':1000,'DIFFUSION_RATE':1,
    'INITIAL_ENERGY':1000,'CHANNEL_COST':1,'DIVISION_THRESHOLD':1000,'BASE_METABOLIC_COST':1,
    'MAINTENANCE_COST':1,'UPTAKE_PER_CHANNEL':1000,'BUILD_PROB_SLOPE':1,'SUPPRESSION_K':1000,
    ...
    'PHAGE_A_LATENT_PERIOD':1000,'INITIAL_PHAGE_A_CONCENTRATION':1000,'PHAGE_B_DIFFUSION_RATE':1,
    'PHAGE_B_ADSORPTION_RATE':1,'PHAGE_B_BURST_SIZE':1000,'PHAGE_B_DECAY_RATE':1,
    'PHAGE_B_LATENT_PERIOD':1000,'INITIAL_PHAGE_B_CONCENTRATION':1000,'PHAGE_C_DIFFUSION_RATE':1,
    'PHAGE_C_ADSORPTION_RATE':1,'PHAGE_C_BURST_SIZE':1000,'PHAGE_C_DECAY_RATE':1,
    'PHAGE_C_LATENT_PERIOD':1000,'INITIAL_PHAGE_C_CONCENTRATION':1000,'MAJOR_MEAL_FRACTION':1
}

# нормализация
normalized_df = df.copy()
for param in param_cols:
    max_val = param_max.get(param, 1)
    normalized_df[param] = df[param] / max_val

# === Вычисляем стандартное отклонение по каждому параметру в каждом кластере ===
cluster_std = normalized_df.groupby(CLUSTER_METHOD)[param_cols].std()
cluster_std['mean_std_across_params'] = cluster_std.mean(axis=1)

# === Визуализация гетерогенности (средние стандартные отклонения) ===
plt.figure(figsize=(8, 5))
sns.barplot(x=cluster_std.index, y=cluster_std['mean_std_across_params'], palette="Set2")
plt.title(f"Cluster Heterogeneity (Mean Normalized Std) — {CLUSTER_METHOD}")
plt.xlabel("Cluster")
plt.ylabel("Mean Normalized Std across Parameters")
plt.tight_layout()
plt.savefig("cluster_heterogeneity_by_std.png", dpi=300)
plt.show()
