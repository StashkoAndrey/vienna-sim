...
// -----------------------------------------------------------------------------
// Phenotype-change model
// -----------------------------------------------------------------------------
//
// 0 = null model:
//     no phenotype change
//
// 1 = true inherited phase variation:
//     stochastic type switching happens during division and is inherited
//
// 2 = rapid phage-induced adaptation:
//     no inherited division switching;
//     phenotype change can happen only after local matching phage pressure is high
//
constexpr int PHASE_MODEL = 1;

// Model 1 parameter:
// inherited stochastic switching during division
constexpr float DIVISION_SWITCH_PROB = 0.01f;

// Model 2 parameters:
// rapid phenotype change after local matching phage pressure
constexpr float RAPID_ADAPTATION_PROB = 0.01f;
constexpr float RAPID_ADAPTATION_PHAGE_THRESHOLD = 10.0f;
