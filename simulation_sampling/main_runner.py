...
def run_replicate(sim_id, rep_id, param_row_dict):
    g = SimulationGrid(GRID_SIZE, param_row_dict)
    centre = GRID_SIZE // 2
    ...
    for step in range(MAX_STEPS):
        ...
        phage_A_rate = min(param_row_dict['PHAGE_A_DIFFUSION_RATE'], 0.15)
        phage_B_rate = min(param_row_dict['PHAGE_B_DIFFUSION_RATE'], 0.15)
        phage_C_rate = min(param_row_dict['PHAGE_C_DIFFUSION_RATE'], 0.15)

        g.phages_A = np.clip(diffuse(g.phages_A, phage_A_rate), 0, None)
        g.phages_B = np.clip(diffuse(g.phages_B, phage_B_rate), 0, None)
        g.phages_C = np.clip(diffuse(g.phages_C, phage_C_rate), 0, None)
